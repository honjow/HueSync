...
class AsusLEDDeviceHID:
    ...
    def __init__(
        ...
        self._dynamic_lighting_disabled = (
            False  # Track if we've disabled dynamic lighting
        )
    ...
    def _disable_ally_x_dynamic_lighting(self) -> None:
        ...
        try:
            ...
            for device in hid_device_list:
                ...
                application = (device.get("usage_page", 0) << 16) | device.get(
                    "usage", 0
                )
                if application != 0x00590001:
                    logger.debug(
                        f"Skipping non-dynamic-lighting interface: application=0x{application:08X}"
                    )
                    continue
                ...
                try:
                    ...
                    dynled_device.close()
                    logger.info(
                        "Disabled Ally X dynamic lighting interface (application 0x00590001)"
                    )
                    ...
                    logger.debug(
                        f"Failed to disable dynamic lighting on {device['path']}: {e}"
                    )
                    continue
        except Exception as e:
    ...
    def set_led_color(
        ...
        if self._last_rgb_config != current_config:
            ...
            try:
                ...
                return (
                    False  # Return failure instead of just logging | 返回失败而非仅记录
                )
    ...
    def set_custom_zone_colors(
        self, colors: list[tuple[int, int, int]], init: bool = False
    ) -> bool:
        """
        ...
        # Check if config_rgb needs to be sent (similar to set_led_color)
        # 检查是否需要发送 config_rgb 命令（与 set_led_color 方法类似）
        current_config = (self.rgb_boot, self.rgb_charging)
        if self._last_rgb_config != current_config:
            logger.debug(
                f"Sending config_rgb command: boot={self.rgb_boot}, charging={self.rgb_charging}"
        ...
        try:
            ...
            # 设备不再可用，清除缓存以便重新连接
            logger.error(
                f"HID device error in set_custom_zone_colors, clearing device cache: {e}",
                exc_info=True,
            )
            self.hid_device = None
            self._last_rgb_config = None
